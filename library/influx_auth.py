...
class InfluxAuth(object):
    ...
    def run(self):
        ...
        result = dict(
            rc=0, failed=False, changed=False, msg="Influxdb authentications ..."
        )
        ...
        authentications = list(map(lambda d: d.get("token", "default value"), _out))
        self.module.log(
            msg=f" - existion authentications: {', '.join(authentications)}"
        )
        ...
        for username, v in self.authentications.items():
            ...
            if _state and v.get("organization", {}).get("admin", False):
                ...
                if rc == 0:
                    ...
                    res = {}
                    res[username] = dict(state=err, failed=True)
                    result_state.append(res)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            authentications=dict(required=True, type="dict"),
        ),
